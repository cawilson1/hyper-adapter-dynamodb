...
export const queryDocuments =
  ...
  async ({ query, db }) => {
    ...
    if (isEmpty(selector)) return queryAll({ ddb, db });
    const params = {
    ...
    console.log(result);
    const value = result?.Items ? s(result.Items) : s(result); //ddb returns inconsistent responses depending on case.
    //dynamodb partiql only supports sorting via primary key values
    const sorted = sort
      ? R.sort((a, b) => (keys[sort[0]][0] === "DESC" ? b - a : a - b))(value)
      : value;
    return sorted;
  };
...
export const indexDocuments =
  ...
  async ({ db, name, fields }) => {
    console.log({ db, name, fields }, marshall(fields));
    const successful = await ddb.updateItem({
      TableName,
