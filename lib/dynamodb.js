...
import { retrieveIndexFields } from "./queryUtils/queryUtils.js";
...
export const queryDocuments =
  ...
  async ({ query, db }) => {
    ...
    const index = await retrieveIndexFields({ db, ddb, use_index });
    ...
    const value = result?.Items
      ? unmarshallResult(result.Items)
      : unmarshallResult(results); //ddb returns inconsistent responses depending on case.
